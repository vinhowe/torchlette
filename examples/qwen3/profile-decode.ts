...
async function main() {
  ...
  const tokens = [...PROMPT];
  let kv: KVCache[] = [];
  const staticKV: StaticKV | undefined =
    mode === "static" ? model.allocStaticKV(512) : undefined;
  {
    const t0 = performance.now();
    const idx = api.tensorFromArray(tokens, [1, tokens.length]);
    const { logits, presentKVs } = api.noGrad(() =>
      model.forward(idx, { staticKV }),
    );
    kv = presentKVs;
  ...
  //
  // Static mode: ceremony-free step-scoped cleanup — each markStep reclaims
  // the interval's graph temporaries deterministically (without this,
  // inference loops leak ~1 storage handle per graph node per token until V8
  // GC collects the wrappers, and the markStep sweep grows unboundedly).
  // Cat mode must NOT enable it: its presentKVs are created inside the
  // interval and held across markStep — they'd be reclaimed (use
  // api.persist(kv.k/v) if you ever need both).
  if (staticKV) api.setStepScopedCleanup(true);
  ...
  for (let i = 0; i < numSteps; i++) {
    ...
    console.log(
      `decode step ${i}${i < WARMUP ? " (warmup)" : ""}: ${dt.toFixed(1)}ms`,
    );
  }
