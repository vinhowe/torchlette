import { w as attr, x as attr_class, y as ensure_array_like, z as stringify } from "./index.js";
...
function getPlanAnalysisGeneration() {
...
function getTimestampWrites(label) {
  if (!_profilingEnabled || !gpuTs.supported || !gpuTs.querySet || !gpuTs.enabled) return void 0;
...
function resolveGpuTimestamps() {
  if (!_profilingEnabled || !gpuTs.supported || !gpuTs.querySet || !gpuTs.resolveBuffer || gpuTs.nextSlot === 0) return;
...
async function readGpuTimestamps() {
  if (!_profilingEnabled || !gpuTs.supported || !gpuTs.device || !gpuTs.querySet || !gpuTs.resolveBuffer || gpuTs.passRecords.length === 0 || gpuTs.stagingSlots === 0) return;
  const { awaitDeferredFence: awaitDeferredFence2 } = await import("./buffer-pool-BANCNxXy.js");
...
var _profilingEnabled, cpuProfile, GPU_MAX_PASSES, gpuTs, _planAnalysisGeneration, padR, padL;
var init_profiler$1 = __esmMin((() => {
  _profilingEnabled = typeof process !== "undefined" && !!define_process_env_default$2?.TORCHLETTE_PROFILE;
  cpuProfile = {
    ...
    */
    snapshotLeakedAllocs(step2) {
      ...
      for (const [, info] of this._allocStacks) {
        ...
        const key = info.stack.split("\n").filter((l) => !l.includes("_trackAllocationInner") && !l.includes("trackAllocation")).slice(0, 4).join("\n");
...
var init_storage_tracker = __esmMin((() => {
  ...
  StorageTracker = class {
    ...
    */
    snapshotForStep() {
      this._stepStartTensors = /* @__PURE__ */ new WeakSet();
      for (const [, ref2] of this.tensorWeakRefs) {
        const target = ref2.deref();
    ...
    */
    destroyStepScoped() {
      ...
      for (const [id, ref2] of this.tensorWeakRefs) {
        const target = ref2.deref();
        if (target === void 0 || !this._stepStartTensors.has(target)) {
          this.externallyReachable.delete(id);
          this.tensorWeakRefs.delete(id);
          this.recentlyUnreachable.add(id);
        }
      }
      for (const id of this.externallyReachable) if (!this.tensorWeakRefs.has(id)) {
        this.externallyReachable.delete(id);
        this.recentlyUnreachable.add(id);
      }
...
function createStorageHandle(device, backendTensor) {
  const storage = {
    id: nextStorageId++,
    device,
    backendTensor
...
function wrapResultAsStorage(device, resultTensor, backendInputs, inputStorages) {
  ...
  if (inputStorages.length > 0) {
    ...
    if (baseIdx < 0 && resultTensor.ownsBuffer === false) {
      const resultBuf = resultTensor.buffer;
      if (resultBuf) {
        for (let i = 0; i < backendInputs.length; i++) if (backendInputs[i].buffer === resultBuf) {
          baseIdx = i;
          break;
        }
      }
    }
    if (baseIdx >= 0) storage.baseStorageId = inputStorages[baseIdx].id;
  }
  return storage;
}
async function ensureWebGPUMatmulImports() {
  if (!_webgpuMatmulImports) {
    const mod = await import("./webgpu-WztPj-Yw.js");
  ...
  if (!_webgpuMatmulGeomImports) {
    const mod = await import("./dispatch-RO6qzdR4.js");
...
function minimum$1(a, b) {
  return applyBinaryOp(a, b, BINARY_OPS.minimum);
}
function maximum$1(a, b) {
...
function stridedScatterImpl$1(opName, base, src, options, combine) {
  ...
  const baseStrides = contiguousStrides(base.shape);
...
function executeAdamStep(node, backendInputs, backend) {
  ...
  return (async () => {
    ...
    node.results = [
      null,
      createStorageHandle(node.device, adamResult.m),
      createStorageHandle(node.device, adamResult.v)
    ];
...
async function requestDeviceWithFallback(adapter, f16Supported, subgroupSupport) {
  ...
  if (isProfilingEnabled()) console.log(`[WebGPU] Adapter limits: maxStorageBufferBindingSize=${adapterMaxStorage}, maxBufferSize=${adapterMaxBuffer}, maxStorageBuffersPerShaderStage=${adapterMaxStorageBuffers}`);
...
function dispatchFusedKernel(device, recipe, inputs, options = {}) {
  ...
  for (const output of recipe.outputs) {
    const buffer = resolveOutputBuffer(device, totalElements * dtypeBytes(output.dtype), inputGPUBuffers);
    trackSharedEncoderWrite(buffer);
...
function makeReductionSpec(config) {
  ...
  const bindings = {};
  if (batched) {
    for (let i = 0; i < count; i++) bindings[`in${i}`] = {
      storage: "read",
      type: "f32"
    };
    for (let i = 0; i < count; i++) bindings[`out${i}`] = {
      storage: "read_write",
      type: "f32"
    };
  } else {
  ...
  return {
    ...
    kernel(ctx) {
      ...
      const load = (i, off) => batched ? ctx.load(`in${i}`, off) : emitLoad(ctx, off);
      const store = (i, val) => ctx.emitStore(batched ? `out${i}` : "out", idx, batched ? val : finalize(ctx, val, idx));
      const reductionSize = ctx.uniform("reductionSize");
      if (reduceOp === "sum") {
        const accs = [];
        for (let i = 0; i < count; i++) accs.push(ctx.emitVar(`t${i}`, "f32", ctx.f32(0)));
        ...
        const accs = [];
        for (let i = 0; i < count; i++) accs.push(ctx.emitVar(`a${i}`, "f32", load(i, firstOff)));
        ctx.forRange(ctx.u32(1), reductionSize, (r) => {
          const off = buildInputOffset(ctx, idx, r, dim);
          for (let i = 0; i < count; i++) {
            const val = load(i, off);
            accs[i].set(reduceOp === "max" ? accs[i].get().max(val) : accs[i].get().min(val));
          }
        });
        for (let i = 0; i < count; i++) store(i, accs[i].get());
      }
...
function batchedReduction(op, inputs, dim, keepdim = false) {
  if (inputs.length === 0) return [];
  if (inputs.length === 1) return [reduction(op, inputs[0], {
    dim,
    keepdim
  })];
  const ctx = requireContext();
  ...
  if (!setup) return inputs.map((t) => reduction(op, t, {
    dim,
    keepdim
  }));
  const { normalizedDims, outShape, outSize, reductionSize, inputStrides, outStrides, inputToOutDim } = setup;
  if (reductionSize > 64) return inputs.map((t) => reduction(op, t, {
    dim,
    keepdim
  }));
  ...
  for (let start = 0; start < tensors.length; start += maxBatch) {
    const batch = tensors.slice(start, Math.min(start + maxBatch, tensors.length));
    const count = batch.length;
    if (count === 1) {
      results.push(reduction(op, inputs[start], {
        dim,
        keepdim
      }));
      continue;
    ...
    const buffers = {};
    for (let i = 0; i < count; i++) buffers[`in${i}`] = batch[i].buffer;
    for (let i = 0; i < count; i++) buffers[`out${i}`] = resolveOutputBuffer(ctx.device, outSize * 4, inputBuffers);
    createTileKernelDispatcher(spec).dispatch(buffers, {
      outSize,
      reductionSize
    });
    for (let i = 0; i < count; i++) results.push(createTensor(outShape, buffers[`out${i}`]));
  }
  return results;
}
...
var unary, binary, add, mul, pow, minimum, maximum, sqrt, relu, exp, log, neg, abs, tanh, sigmoid, silu, sin, cos, rsqrt, floor, ceil, round, sign, isfinite;
...
function emitAdamScalarBody(ctx, idx, gVar, emitF16, suffix = "") {
  ...
  const g = gVar.get();
  const mNew = ctx.emitLet(`m_new${suffix}`, beta1.mul(ctx.load("m_in", idx)).add(ctx.f32(1).sub(beta1).mul(g)));
  const vNew = ctx.emitLet(`v_new${suffix}`, beta2.mul(ctx.load("v_in", idx)).add(ctx.f32(1).sub(beta2).mul(g).mul(g)));
...
async function adamStep(grad, param, m, v, config) {
  ...
  try {
    ...
    if (paramBuf === gradBuf || paramBuf === mBuf || paramBuf === vBuf) {
      ...
      if (enc) enc.copyBufferToBuffer(paramBuf, 0, dst, 0, bufSize);
      else {
...
function startScalarReadback(a) {
  const ctx = requireContext();
  const tensor = asGPUTensor(a);
  const bytesPerElement = dtypeBytes(tensor.dtype);
  const alignedBytes = alignBufferSize(tensor.size * bytesPerElement);
  ...
  return async () => {
    ...
    switch (dtype) {
      case "f16":
        value = f16ArrayToF32Array(new Uint16Array(mapped.slice(0, 2)))[0];
        break;
      case "i32":
...
var webgpu_exports = /* @__PURE__ */ __exportAll({
  ...
  initGpuTimestamps: () => initGpuTimestamps,
...
function buildCandidateGroups(nodes, excludedIds) {
...
function batchGlobalSingletons(nodes, finalGroups, excludedIds, maxBuffers) {
  const groupedNodeIds = /* @__PURE__ */ new Set();
  for (const g of finalGroups) for (const n of g.nodes) groupedNodeIds.add(n.id);
  const singletons = [];
  for (let i = 0; i < nodes.length; i++) {
    const node = nodes[i];
    if (isFusibleOp(node.op) && !groupedNodeIds.has(node.id) && !excludedIds?.has(node.id)) singletons.push({
      node,
      planIndex: i
    });
  }
...
function detectFusionGroups(nodes, externalNodeIds, options) {
  ...
  const { candidateGroups, fusibleCount } = buildCandidateGroups(nodes, excludedIds);
...
function mergeBatchableReductions(actions, planNodes) {
  ...
  for (const [, members] of groups) {
    ...
    for (let i = firstIdx + 1; i < lastIdx && safe; i++) {
      ...
      for (const ni of nodeIndices) {
        ...
        if (!node) continue;
        for (const inp of node.inputs) if (inp.kind === "pending" && groupNodeIndices.has(inp.node.id)) ;
      }
    }
    safe = true;
    for (let mi = 0; mi < members.length - 1 && safe; mi++) {
      ...
      for (let ai = members[mi].actionIndex + 1; ai <= lastIdx; ai++) {
        for (const ni of getActionNodeIndices(actions[ai])) {
          ...
          if (!depNode) continue;
          for (const inp of depNode.inputs) if (inp.kind === "pending" && inp.node === memberNode) {
            safe = false;
            break;
          }
          if (!safe) break;
    ...
    for (let mi = 0; mi < members.length - 1; mi++) actions[members[mi].actionIndex] = {
      kind: "prologue-skip",
      nodeIndex: members[mi].nodeIndex
    };
  }
}
function getActionNodeIndices(action) {
  switch (action.kind) {
    case "sequential":
    case "view":
    case "data-source":
...
const REDUCTION_OPS = /* @__PURE__ */ new Set([
  "sum",
  "max",
  "min",
  "mean"
]);
function isReductionOp(op) {
  ...
  for (let nodeIdx = 0; nodeIdx < nodes.length; nodeIdx++) {
    ...
    if (isReductionOp(node.op) && node.payload?.dim != null) {
      ...
      for (let j = nodeIdx + 1; j < nodes.length; j++) {
        const next = nodes[j];
        if (next.op === node.op && !next.result && next.payload?.dim != null && reductionDimKey(next) === dimKey) batchCount++;
        else break;
      }
      if (batchCount > 1) {
        const indices = [];
        for (let c = 0; c < batchCount; c++) indices.push(posMap.get(nodes[nodeIdx + c].id));
        actions.push({
          kind: "batched-reduction",
          nodeIndices: indices,
          reduceOp: node.op,
          payload: {
            dim: node.payload.dim,
            keepdim: node.payload.keepdim
          }
        });
...
function analyzeGraph(planNodes, externalNodeIds, maxStorageBuffers) {
  ...
  if (typeof process !== "undefined" && define_process_env_default?.TORCHLETTE_DEBUG_FUSION === "1" && reorderedNodes.length > 200 && matmulEpilogueChains.size > 0) {
    console.log(`[fusion-debug] ${reorderedNodes.length} nodes: ${matmulEpilogueChains.size} matmul epilogue chains`);
    for (const [mmId, chainIds] of matmulEpilogueChains) {
      const mm = nodeById.get(mmId);
      const chainOps = chainIds.map((id) => {
        const n = nodeById.get(id);
        return n ? `${n.op}(${JSON.stringify(n.shape)})` : `?#${id}`;
      }).join(" → ");
      console.log(`  matmul#${mmId} ${JSON.stringify(mm?.shape)} → ${chainOps}`);
    }
  }
  ...
  if (typeof process !== "undefined" && define_process_env_default?.TORCHLETTE_DEBUG_FUSION === "1" && reorderedNodes.length > 200) {
    for (const n of reorderedNodes) if (n.op === "gelu") {
      ...
      if (epi || pro || row || rew) console.log(`[fusion-debug] gelu#${n.id} ${JSON.stringify(n.shape)} claimed by: ${epi ? "epilogue " : ""}${pro ? "prologue " : ""}${row ? "rowProgram " : ""}${rew ? "rewrite" : ""}`);
    }
  }
  let allClaimedIds;
  ...
  if (typeof process !== "undefined" && define_process_env_default?.TORCHLETTE_DEBUG_FUSION === "1" && reorderedNodes.length > 200) {
    ...
    const runLengths = [];
    let runLen = 0;
    for (const n of reorderedNodes) if (isFusibleOp(n.op) && !allClaimedIds?.has(n.id)) {
      totalFusible++;
      runLen++;
    } else {
    ...
    let fusedNodes = 0;
    for (const seg of segments) if (seg.kind === "fused" && seg.group.nodes.length >= 2) fusedNodes += seg.group.nodes.length;
    console.log(`
[fusion-debug] ${reorderedNodes.length} nodes, ${totalFusible} fusible (unclaimed), ${fusedNodes} fused`);
    console.log(`  Consecutive runs (top 15): ${runLengths.slice(0, 15).join(", ")}`);
    let breaks = 0;
    for (let i = 1; i < reorderedNodes.length && breaks < 8; i++) {
      ...
      const prevIs3072 = prev.shape.length >= 2 && prev.shape[prev.shape.length - 1] === 3072;
      if (prevFusible && !curFusible && prevIs3072) {
        breaks++;
        console.log(`  Break #${breaks} at pos ${i}: ${cur.op} ${cur.dtype} ${JSON.stringify(cur.shape)}${allClaimedIds?.has(cur.id) ? " [CLAIMED]" : ""}${externalNodeIds?.has(cur.id) ? " [EXTERNAL]" : ""}`);
        for (let j = Math.max(0, i - 2); j < Math.min(reorderedNodes.length, i + 3); j++) {
          const n = reorderedNodes[j];
          const f = isFusibleOp(n.op) ? "F" : ".";
          const c = allClaimedIds?.has(n.id) ? "C" : " ";
          const e = externalNodeIds?.has(n.id) ? "E" : " ";
          console.log(`    [${j}] ${f}${c}${e} ${n.op.padEnd(14)} ${n.dtype.padEnd(4)} ${JSON.stringify(n.shape)}`);
        }
      }
    }
    ...
    for (const [shape, g] of [...shapeGroups.entries()].sort((a, b) => b[1].count - a[1].count)) {
      const ops = [...g.ops.entries()].sort((a, b) => b[1] - a[1]).map(([o, c]) => `${o}:${c}`).join(" ");
...
async function executePlanSequential(plan, backend, options) {
  ...
  if (options?.enableEarlyRelease) {
    let externalNodeIds;
    try {
      const { getPendingNodeIds: getPendingNodeIds2 } = await Promise.resolve().then(() => tensorDNI3XZeL);
...
async function ensureFusionImports() {
  ...
  _cachedDispatchFusedKernel = (await import("./fusion-dispatch-BLS1s3O4.js")).dispatchFusedKernel;
  await ensureWebGPUMatmulImports();
...
async function executeRowProgram(program, inputRefs, outputNode, numRows, dimSize, coveredNodes, backend) {
  ...
  try {
    const { dispatchRowProgram } = await import("./row-program-dispatch-HNv20MIc.js");
...
async function executeAdamBatchInner(planNodes, nodeIndices, adamOp, getStorage) {
  ...
  const handled = dispatchPackedOptimizer({
    ...
    dispatch(packed, totalElements) {
      const result = dispatchAdamStep(packed[0], packed[1], packed[2], packed[3], totalElements, config, false, infFlagBuffer);
      const enc = getSharedEncoderInstance();
      if (enc) {
...
function assignPerParamAdamResult(adamNode, _s1, adamResult) {
...
async function buildAndCacheDispatchConfig(action, resolvedInputRefA, resolvedInputRefB, inputCastA, inputCastB, inputAPath, inputBPath, epilogueInputRefs, epilogueInputPaths) {
  ...
  const { isF16Supported: isF16Supported2 } = await import("./webgpu-WztPj-Yw.js");
      ...
      switch (action.kind) {
        ...
        case "batched-reduction": {
          ...
          const batchInputs = batchNodes.map((node) => {
            return getInputStorage(node.inputs[0], batchBackend).backendTensor;
          });
          const batchedSum = batchBackend.ops.batchedReduction;
          if (batchedSum) {
            const results = batchedSum(action.reduceOp, batchInputs, action.payload.dim, action.payload.keepdim);
            for (let bi = 0; bi < batchNodes.length; bi++) batchNodes[bi].result = wrapResultAsStorage(batchNodes[bi].device, results[bi], [batchInputs[bi]], []);
            stats.sequentialNodes += batchNodes.length;
          } else for (let bi = 0; bi < batchNodes.length; bi++) {
            const node = batchNodes[bi];
            const inputs = node.inputs.map((ref2) => getInputStorage(ref2, batchBackend));
            const backendInputs = inputs.map((s) => s.backendTensor);
            const result = executeOpSync(node, backendInputs, batchBackend);
            node.result = wrapResultAsStorage(node.device, result instanceof Promise ? await result : result, backendInputs, inputs);
...
async function executePlanOptimized(plan, backend, options = {}) {
  ...
  ops: ${opSummary}`);
    ...
    const pa = collectProfilingStats(analysis.segments, analysis.epilogueClaimedIds, analysis.prologueClaimedIds, analysis.matmulEpilogueChains, plan.nodes.length);
...
function collectProfilingStats(segments, excludedIds, prologueClaimedIds, matmulEpilogueChains, totalNodes) {
  let fusedSegCount = 0;
  let seqSegCount = 0;
  ...
  for (const segment of segments) if (segment.kind === "fused" && segment.group.nodes.length >= 2) {
    ...
    seqSegCount++;
    for (const node of segment.nodes) if (!excludedIds.has(node.id) && !prologueClaimedIds.has(node.id)) recordUnfused(node);
...
function collectTensorHandles(value) {
  ...
  const visit = (current) => {
    ...
    if (typeof obj._engineTensor === "function") {
      const handle = obj._engineTensor();
      if (handle instanceof EngineTensor) out.push(handle);
      return;
    }
...
var TidyDispatchMode = class {
  ...
  disposeNonEscaped() {
    let disposed = 0;
    for (const t of this.tracked) if (!this.escaped.has(t) && !t.disposed) {
      t.dispose();
      disposed++;
    }
    this.tracked.clear();
...
var RuntimeEngine = class {
  ...
  *  unreachable after backward clears autograd graph references. */
  _etFinalizer = new FinalizationRegistry((baseId) => {
    const count = this._basePinCount.get(baseId) ?? 0;
    if (count <= 1) this._basePinCount.delete(baseId);
    else this._basePinCount.set(baseId, count - 1);
  });
  _rngBasis = {
    algorithmId: 0,
    seed: 0
  ...
  */
  async forceAllMerged(...tensors) {
    ...
    const { materializePendingTensors: materializePendingTensors2, clearDisposedPendingNodeIds: clearDisposedPendingNodeIds2 } = await Promise.resolve().then(() => tensorDNI3XZeL);
  ...
  */
  async forceAllPending() {
    const { getAllPendingTensors: getAllPendingTensors2, materializePendingTensors: materialize, clearDisposedPendingNodeIds: clearDisposedPendingNodeIds2 } = await Promise.resolve().then(() => tensorDNI3XZeL);
    ...
    storageTracker.getNextStorageId();
    const backend = this.getBackend(device);
  ...
  */
  async startItemReadback(a) {
    await this.force(a);
    const backend = this.getBackend(a.device);
    if (backend.ops.startScalarReadback) return backend.ops.startScalarReadback(a.backendTensor);
  ...
  */
  ...
  zero_(dst) {
    const srcNode = createLazyIRNode("tensorFromArray", [], dst.shape, dst.dtype, dst.device, { values: new Float32Array(dst.shape.reduce((a, b) => a * b, 1)) });
  ...
  fill_(dst, value) {
    ...
    values.fill(value);
    const srcNode = createLazyIRNode("tensorFromArray", [], dst.shape, dst.dtype, dst.device, { values });
    return this._scatterFromNode(dst, srcNode);
  }
  mul_(dst, value) {
    const { refA, refB, shape, dtype, device } = this.resolveBinaryOp("mul", dst, value);
  ...
  _scatterFromNode(dst, srcNode) {
    const payload = {
      offset: 0,
      viewShape: dst.shape.slice(),
      viewStrides: contiguousStrides(dst.shape)
    };
    const scatterNode = createLazyIRNode("stridedScatterCopy", [dst.lazyRef, createPendingRef(srcNode)], dst.shape, dst.dtype, dst.device, payload);
  ...
  */
  ...
  async markStep() {
    this._debug_runEntryPoint(() => {
      for (const [baseId] of this._baseState) if (!this._basePinCount.has(baseId)) this._baseState.delete(baseId);
      for (const [, state] of this._baseState) state.committed.clear();
      this._savedTensors.clear();
    });
  }
  _acquireExecLock() {
...
for (const op of [
  ...
  "min",
  "mean"
]) RuntimeEngine.prototype[op] = function(a, options) {
  return this._reductionOp(op, a, options);
};
for (const op of ["argmax", "argmin"]) RuntimeEngine.prototype[op] = function(a, options) {
...
const _pendingCheckpointScopes = [];
function disposeCheckpointIntermediates() {
  for (const scope of _pendingCheckpointScopes) for (const t of scope) if (!t.isDisposed) t.dispose();
...
async function collectAndForceSavedTensors(torch2, ordered) {
  ...
  const recomputeScope = /* @__PURE__ */ new Set();
  torch2._checkpointRecomputeTensors = recomputeScope;
  ...
  try {
    ...
    torch2._checkpointRecomputeTensors = null;
    if (recomputeScope.size > 0) _pendingCheckpointScopes.push(recomputeScope);
  }
  const allPending = [];
...
function cleanupAutogradGraph(torch2, ordered, gradMap) {
  ...
  for (const [tensor, _grad] of gradMap) if (tensor.requiresGrad && !tensor._gradNode() && !tensor.disposed) preserved.add(tensor);
  for (const node of ordered) for (const input of node.inputs) {
    ...
    if (torch2._compileCreatedTensors.has(input)) continue;
    preserved.add(input);
  }
  ...
  const processNode = (node) => {
    ...
    for (const input of node.inputs) {
      if (!preserved.has(input) && !input.disposed) toDispose.add(input);
      const inputNode = input._gradNode();
      if (inputNode && !visitedNodes.has(inputNode)) processNode(inputNode);
    }
    node.savedSlots.length = 0;
    node.output._setGradNode(null);
    node.inputs.length = 0;
  };
  for (const node of ordered) processNode(node);
...
function matmulBackward(ctx, aShape, bShape) {
...
function linearBackward(ctx, inputShape, weightShape, needsInputGrad, needsWeightGrad, hasBias) {
  return (grad, getSaved) => {
...
function geluTanhBackward(ctx) {
  return (grad, getSaved) => {
    ...
    const clampedInner = rt.where(rt.lt(innerVal, -10), -10, rt.where(rt.gt(innerVal, 10), 10, innerVal));
...
function geluErfBackward(ctx) {
  return (grad, getSaved) => {
    ...
    const poly = rt.add(rt.mul(0.254829592, t), rt.add(rt.mul(-0.284496736, t2), rt.add(rt.mul(1.421413741, t3), rt.add(rt.mul(-1.453152027, t4), rt.mul(1.061405429, t5)))));
...
function layernormBackwardImpl(torch2, grad, getSaved, normalizedDim, lastDimSize, rank2, eps = 1e-5) {
  ...
  if (savedX.device === "webgpu") {
    ...
    const sumDims = Array.from({ length: rank2 - 1 }, (_, i) => i);
    gradBias = torch2.runtime.sum(grad, {
      dim: sumDims,
      keepdim: false
    });
    gradWeight = torch2.runtime.sum(torch2.runtime.mul(grad, recomputeNormalized), {
      dim: sumDims,
      keepdim: false
    });
...
var Torchlette = class {
  ...
  */
  ...
  matmul(a, b) {
    ...
    return this._wrapWithGrad(inner, [a, b], matmulBackward(this._backwardCtx(), aShape, bShape), tensorsToSave);
  }
  /**
  ...
  */
  linear(input, weight, bias = null) {
    ...
    return this._wrapWithGrad(inner, allInputs, linearBackward(this._backwardCtx(), inputShape, weightShape, needsInputGrad, needsWeightGrad, !!bias), toSave);
  }
  gelu(a, options) {
    this._assertUsable(a);
    const approximate = options?.approximate ?? "tanh";
    ...
    const bwdFn = approximate === "tanh" ? geluTanhBackward(this._backwardCtx()) : geluErfBackward(this._backwardCtx());
  ...
  */
  ...
  _checkpointRecomputeTensors = null;
  _wrap(inner, requiresGrad = false) {
    ...
    if (this._checkpointRecomputeTensors) this._checkpointRecomputeTensors.add(tensor);
    return tensor;
  }
  _wrapWithGrad(inner, inputs, backward, tensorsToSave = []) {
    ...
    if (requiresGrad) {
      for (const tensor of inputs) this.keep(tensor);
      const savedSlots = [];
  ...
  _sumToShape(grad, shape) {
    ...
    const dims = [];
    for (let axis = 0; axis < gradRank; axis += 1) if (axis < pad) dims.push(axis);
    else if (paddedTarget[axis] === 1 && grad.shape[axis] !== 1) dims.push(axis);
    let reduced = grad;
    if (dims.length > 0) {
...
function clipGradNorm_(api, parameters, maxNorm, normType = 2) {
  const grads = [];
  ...
  api.tidy(() => {
    ...
    if (normType === Infinity) {
      ...
      for (const g of grads) {
        ...
        maxTensor = maxTensor === null ? absMax : api.max(api.add(api.mul(api.gt(absMax, maxTensor), absMax), api.mul(api.gt(maxTensor, absMax), maxTensor)));
      }
    ...
    const clipCoef = api.minimum(api.div(maxNorm, api.add(totalNormTensor, 1e-6)), 1);
    for (const g of grads) api.copy_(g, api.mul(g, clipCoef));
  });
}
...
var Embedding$1 = class Embedding extends Module {
  numEmbeddings;
  embeddingDim;
...
var LayerNorm$1 = class LayerNorm extends Module {
  normalizedShape;
  eps;
...
var Linear$1 = class Linear extends Module {
  inFeatures;
  outFeatures;
...
init_webgpu();
init_storage_tracker();
const { checkpoint, crossEntropy, Embedding: Embedding2, LayerNorm: LayerNorm2, Linear: Linear2 } = nn_exports;
let status = "idle";
const modelStore = {
  get isReady() {
    return status === "ready";
...
const TINY_SHAKESPEARE_URL = "https://raw.githubusercontent.com/karpathy/char-rnn/master/data/tinyshakespeare/input.txt";
const DATASETS = [
  {
    id: "shakespeare",
    label: "Shakespeare",
...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    if (trainingStore.lossHistory.length >= 2) {
      $$renderer2.push("<!--[-->");
      $$renderer2.push(`<div class="grid grid-cols-2 gap-1"><div class="border border-slate-800 p-1"><svg viewBox="-2 -2 104 104" class="w-full h-20" preserveAspectRatio="none"><path${attr("d", lossChartPath(trainingStore.lossHistory))} fill="none" stroke="#3b82f6" stroke-width="1.5" vector-effect="non-scaling-stroke"></path></svg> <div class="flex justify-between text-[9px] font-mono text-slate-600 px-0.5"><span>${escape_html(Math.min(...trainingStore.lossHistory).toFixed(2))}</span> <span>loss (${escape_html(trainingStore.lossHistory.length)})</span> <span>${escape_html(Math.max(...trainingStore.lossHistory).toFixed(2))}</span></div></div> `);
      if (trainingStore.memoryHistory.length >= 2) {
        $$renderer2.push("<!--[-->");
        $$renderer2.push(`<div class="border border-slate-800 p-1"><svg viewBox="-2 -2 104 104" class="w-full h-20" preserveAspectRatio="none"><path${attr("d", chartPath(trainingStore.memoryHistory))} fill="none" stroke="#f59e0b" stroke-width="1.5" vector-effect="non-scaling-stroke"></path></svg> <div class="flex justify-between text-[9px] font-mono text-slate-600 px-0.5"><span>${escape_html(Math.min(...trainingStore.memoryHistory).toFixed(0))}</span> <span>MB</span> <span>${escape_html(Math.max(...trainingStore.memoryHistory).toFixed(0))}</span></div></div>`);
      } else {
        $$renderer2.push("<!--[!-->");
      }
      $$renderer2.push(`<!--]--></div>`);
    ...
    for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
      let ds = each_array[$$index];
      $$renderer2.push(`<button${attr_class(`px-1.5 py-0.5 text-[10px] rounded ${stringify(trainingStore.dataSource?.includes(ds.label) ? "bg-blue-600 text-white" : "bg-slate-800 text-slate-400 hover:bg-slate-700 hover:text-slate-300")}`)}${attr("disabled", trainingStore.dataLoading, true)}>${escape_html(ds.label)}</button>`);
    }
    $$renderer2.push(`<!--]--></div></div></div> <div class="p-2 flex flex-col gap-2"><div class="text-[10px] uppercase tracking-widest text-slate-500 font-semibold">Generate</div> <textarea class="w-full bg-slate-900 border border-slate-700 text-sm text-slate-200 font-mono p-1.5 resize-none focus:outline-none focus:border-blue-600" rows="2" placeholder="Enter prompt..."${attr("disabled", generationStore.isGenerating, true)}>`);
    const $$body = escape_html(generationStore.prompt);
    if ($$body) {
      $$renderer2.push(`${$$body}`);
    }
    $$renderer2.push(`</textarea> <div class="flex items-center gap-3">`);
    {
      $$renderer2.push("<!--[!-->");
      $$renderer2.push(`<button${attr("disabled", true, true)} class="px-2 py-0.5 text-xs bg-blue-600 hover:bg-blue-500 text-white disabled:opacity-30 disabled:cursor-not-allowed">Generate</button>`);
    }
    $$renderer2.push(`<!--]--> <div class="flex items-center gap-2 text-xs text-slate-500"><label class="flex items-center gap-1">temp <input type="range" min="1" max="200"${attr("value", Math.round(generationStore.temperature * 100))} class="w-16"/> <span class="font-mono text-slate-400 w-6">${escape_html(generationStore.temperature.toFixed(1))}</span></label> <label class="flex items-center gap-1">k <input type="range" min="1" max="100"${attr("value", generationStore.topK)} class="w-12"/> <span class="font-mono text-slate-400 w-5">${escape_html(generationStore.topK)}</span></label></div></div> <div class="flex-1 min-h-[200px] border border-slate-800 p-2 font-mono text-xs text-slate-300 overflow-y-auto whitespace-pre-wrap">`);
    {
      $$renderer2.push("<!--[!-->");
      {
        $$renderer2.push("<!--[!-->");
...
const tensorDNI3XZeL = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
