#!/usr/bin/env npx tsx
/**
 * Test GPT2 without MLP to isolate the NaN issue.
 */
...
async function main(): Promise<void> {
  ...
  ln1.loadWeights(
    api.tensorFromArray(ln1W.data, ln1W.shape),
    api.tensorFromArray(ln1B.data, ln1B.shape)
  );
  ...
  lnF.loadWeights(
    api.tensorFromArray(lnFW.data, lnFW.shape),
    api.tensorFromArray(lnFB.data, lnFB.shape)
  );
  ...
  console.log(`\n${'='.repeat(60)}`);
  console.log('GPT2 WITHOUT MLP (attention only)');
  ...
  // Block without MLP
  const normed = ln1.forward(x);
  await checkStats('ln1(x)', normed);

  const attnOut = attn.forward(normed);
  await checkStats('attn(ln1(x))', attnOut);

  x = api.add(x, attnOut); // Residual
  await checkStats('x + attn', x);

  // Final layer norm
  x = lnF.forward(x);
  await checkStats('lnF(x)', x);
