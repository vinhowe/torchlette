/**
 * Focused α sweep on NAMED Gemma Scope features (interpretations from
 * Neuronpedia) to pick a coherent steering α for the demo presets.
 *
 * Run: VULKAN_DEVICE_INDEX=N LD_LIBRARY_PATH=tools/vk-shim TORCHLETTE_STEP_TAPE=1 \
 *      npx tsx examples/gemma2-sae/preset-refine.ts
 */
...
// Named features (Neuronpedia interpretations) worth trying as presets.
const NAMED: { feature: number; name: string }[] = [
  { feature: 3124, name: "San Francisco Bay Area" },
  { feature: 8993, name: "banking / finance" },
  { feature: 12082, name: "dogs (from dog theme)" },
  { feature: 3767, name: "ocean/water (from ocean theme)" },
  { feature: 11571, name: "anger (from anger theme)" },
  { feature: 3419, name: "anger #2 (from anger theme)" },
];
const ALPHAS = [0, 40, 80, 120, 160];
const PROMPTS = [
...
async function main() {
  ...
  const gen = async (feature: number, alpha: number, prompt: string) => {
    ...
    await generateChat(
      ...
      { maxNewTokens: 36, temperature: 0, topK: 40, topP: 0.95, chat: false, residualHook: hook },
    );
    return out;
  };

  for (const { feature, name } of NAMED) {
    console.log(`\n##### #${feature} — ${name} #####`);
    for (const prompt of PROMPTS) {
      console.log(`  prompt: ${JSON.stringify(prompt)}`);
      for (const alpha of ALPHAS) {
        const out = await gen(feature, alpha, prompt);
        console.log(`    α=${alpha.toString().padStart(3)}: ${JSON.stringify(out.slice(0, 180))}`);
      }
    }
  }
  console.log("\nDONE");
