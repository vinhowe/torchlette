...
## §5.4 DESIGN RE-REVIEW — the elementwise/reductions INTERPRETER cutover (2026-07-18): REJECTED by the campaign's own bar
Investigated, not landed (no commits; the honest outcome). Findings: (1) the doc's "ideal shape" (serialize-then-execute per node) is REJECTED for the live path — the serializer walker is built on the replay-only slot substrate, and per-node stream construction on the hottest ops is a near-certain ms/step regression (the hard STOP). (2) The single source ALREADY EXISTS at the tile-dispatch altitude: createTileKernelDispatcher's .plan()/.dispatch() + the shared plan-builders ARE the realizer layer both consumers use — the interpreter is not a stranded mirror; for elementwise the live emit is already the compact declaration-shaped dispatchElementwise, and a walker there is pure indirection (zero deletion). (3) Reductions' cleanly-deletable subset is ≈ −20 net, gated on a net-new live-plan-executor primitive — fails (net-negative ∧ no-unwarranted-mechanism). CONSEQUENCE: the campaign's net-negative promise rests on P2-P4 family cutovers (matmul 1510+205, fused 1013+505, dma 934+248), NOT on interpreter cutovers for families whose dispatch is already realizer-shaped. The named unblock if a future session wants the ≈ −20: one executeTilePlan(plan, buffers) live primitive + shadow, accepted explicitly as mechanism-for-deletion. Elementwise stays as-is by design.
