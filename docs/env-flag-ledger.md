...
| ~~`TORCHLETTE_DERIVED_COMPOSITE_BWD`~~ | **DELETED (C3, 2026-07-23)** | — | Born C2 (routed the CPU rmsnorm/layernorm backwards through the derived `vjpComposition`, hand closures as the flag-off oracle); its born-with-sunset fired at C3 — the flag, the read (`useDerivedCompositeBwd`), AND the hand `rmsnormBackwardImpl`/`layernormBackwardImpl` CPU branch were removed once the derived VJP became the sole CPU realization (fused GPU kernels asserted against it by `composite-fused-vs-derived.spec`). No `ENV.TORCHLETTE_DERIVED_COMPOSITE_BWD` read remains in `src/`. The softmax CPU closure stays hand (C1 T1 — derived measured heavier, reference-only). |
| ~~`TORCHLETTE_DERIVED_ACTIVATION`~~ | **DELETED (A3, 2026-07-23)** | — | Born A2 (routed `applyFusedOp`'s activation cases through the `Expr→BlockExpr` fold `lowerExprToTileIR`, hand switch bodies as the flag-off oracle); its born-with-sunset fired at A3 — the flag, the read, the hand activation switch bodies, AND the `BlockExpr.sigmoid`/`.erf` compound methods were removed once the fold became the sole realization. Surviving guard: `tools/expr-fold-parity.ts` (fold-GPU vs interpret-CPU). No `ENV.TORCHLETTE_DERIVED_ACTIVATION` read remains in `src/`. |
