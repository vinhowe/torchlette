...
| ~~`TORCHLETTE_DERIVED_ADAM`~~ | **DELETED (R4, 2026-07-22)** | — | Landed at R2 (derived fused Adam body behind the flag, authored body as differential oracle); its born-with-sunset (design §6) fired at R4 — the flag, the fold-routed path, AND the authored body's differential arm were removed once the derived body became the sole realization. No `ENV.TORCHLETTE_DERIVED_ADAM` read remains in `src/`. See docs/derived-optimizer-realizer-design.md "R4 — the deletion (LANDED 2026-07-22)". |
| `TORCHLETTE_DERIVED_COMPOSITE_BWD` | **born 2026-07-23 (COMPOSITE-CLOSURE C2)**; off at birth (dark), on at C2 → default at C3 → opt-out dies | `decomposed-ops.ts` (C2) | Routes the CPU / non-fused composite backwards (rmsnorm, layernorm-CPU branch) through the derived `vjpComposition` reverse pass (F1); the hand closures stay as the differential oracle while the flag is off. **Sunset:** dies at C3 (soak → default → the hand CPU closures + the flag are removed once the derived path is the sole realization), mirroring `TORCHLETTE_DERIVED_ADAM`. NOTE: the softmax CPU closure is NOT routed — the C1 cost probe (`comp-adjoint-cost.ts`) measured the derived softmax backward heavier (22 vs 13 nodes, ~1.1× time), so per the design's T1 gate it stays and the derived form is reference-only. |
| `TORCHLETTE_DERIVED_ACTIVATION` | **born 2026-07-23 (COMPOSITE-CLOSURE A2)**; off at birth (dark), on at A2 → default at A3 → opt-out dies | `fusion-tile-ir.ts`, `tile-ir.ts` (A2) | Routes both backend forward-activation sites (the `fusion-tile-ir` activation switch + `tile-ir` `BlockExpr.sigmoid`/`.erf`) through the `Expr→BlockExpr` fold `lowerExprToTileIR` (F2); the hand DSL bodies stay as the byte-differential oracle while off. **Sunset:** dies at A3 (the hand bodies + the flag are removed once the fold is the sole realization). |
| `TORCHLETTE_PACK_OPTIM` | on (Lion/SGD, >1 param, WebGPU); `=0` = per-param | `lion.ts`, `sgd.ts` | **ROLE CHANGED (derived-optimizer-realizer R5c):** now selects the FUSED `optStep` kernel path (Lion/SGD folded through the generic program-roles realizer, `opt-step-realizer.ts` — one DMA-packed in-place dispatch per size class) vs the per-param reference. `=0` forces the per-param path on every backend (the unpacked control arm of `parity-packed-vs-unpacked.ts` AND the CPU / non-fused fallback). The old chain-packing `packOptimizerClass` graph-cat path this flag used to gate for Lion/SGD is DELETED at R5c (the fused kernel is the packed realization; `packOptimizerClass` survives only as Adam's foreach path). **Sunset:** the opt-out dies once nothing load-bearing still compares against the per-param arm — the per-param reference is currently load-bearing (`parity-packed-vs-unpacked`'s unpacked arm + the fold-lemma differential), so the flag stays. |
| `TORCHLETTE_PACKED_OPT` | on; `=0` = per-item | `ops/fused.ts:302` | Packed-dispatch measurement kill switch (the fused multi-item `optStep` path; per-item dispatches when off). Renamed here from the stale `TORCHLETTE_PACKED_ADAM` label — src has only ever read `TORCHLETTE_PACKED_OPT`. |
| `TORCHLETTE_PACK_MAX_BYTES` | unset; `=<bytes>` overrides the pack chunk-size cap | `pack-optimizer.ts:305` | Test/measurement seam: overrides the DMA-pack byte budget so a spec can force multiple pack chunks on a small model (the packer's size-class chunking is otherwise only exercised at 124M scale). **Sunset:** dies with the pack-optimizer chunking test that needs it — born-with-sunset (has no production role). |
| `TORCHLETTE_DONATION` | on; `=0` disables buffer donation | `segment-executors.ts:269` | Donation kill switch. |
| `TORCHLETTE_PLANNER_DONATION` | **off (opt-in)**; `=1` = planner-level donation edge | `stream-generate.ts` (generateFused) | Buffer-donation P2 (docs/buffer-donation-design.md). Plan-build slot-collapse: the generated-stream fused donation edge, un-refused for arena/planner-registry temps. **STOPPED (2026-07-21): recovers 0 % of the packed premium** — the packed-optimizer plans are `fullyCovered=false` (they run on the RECORDED compiled build, not the generated stream, so the edge is never live) AND the packed buffers are oversized (>128 MB → chunked, no in-kernel donation). Inert (never fires) on real training; held opt-in as the ratified P2 edge pending a stage-4 coverage campaign. **Sunset:** dies at P3 flip (folds into a covered-plan default) OR is removed if the coverage prerequisite is abandoned. |
