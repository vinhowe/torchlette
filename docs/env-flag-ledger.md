...
| `TORCHLETTE_DERIVED_ADAM` | **off (opt-in)**; `=1` = derived fused body | `adam.ts` (`_stepFused`), `adam-skeleton.ts` (`realizeAdamStepSpec`), `adam-kernel.ts` | Derived-optimizer-realizer R2 (docs/derived-optimizer-realizer-design.md, ruling O1). `=1` routes the fused Adam kernel's per-element body through the `OptTerm→tile-IR` fold (`lowerOptTermToTileIR(ADAMW_PROGRAM)`) instead of the authored `emitAdamScalarBody`, and binds bias correction as a `[2]` `bc=[bc1,bc2]` scalar-DATA input (fork B — the in-kernel `expm1` prelude dies). Off-by-default keeps the authored body as the differential oracle. **Sunset:** R3 flips it ON (default), R4 DELETES both the flag and the authored body — born-with-sunset (design §6). |
