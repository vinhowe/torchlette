...
describe("DistilGPT2 Checkpoint Memory Verification", () => {
  ...
  it(
    "checkpoint reduces peak memory on transformer forward/backward",
    { timeout: 60000 },
    async () => {
      if (!webgpuAvailable) {
      ...
      console.log(
        `Peak memory WITHOUT checkpoint: ${(peakWithoutCheckpoint / 1e6).toFixed(2)} MB`,
      );
      ...
      console.log(
        `Peak memory WITH checkpoint: ${(peakWithCheckpoint / 1e6).toFixed(2)} MB`,
      );
  ...
  it(
    "checkpoint and non-checkpoint produce same gradient shapes",
    { timeout: 60000 },
    async () => {
      if (!webgpuAvailable) {
        console.log("Skipping: WebGPU not available");
  ...
  it(
    "all parameters receive non-zero gradients with checkpoint",
    { timeout: 60000 },
    async () => {
      if (!webgpuAvailable) {
        console.log("Skipping: WebGPU not available");
      ...
      console.log(
        `Parameters with non-null grads: ${nonNullGrads}/${params.length}`,
      );
      console.log(
        `Parameters with non-zero grads: ${nonZeroGrads}/${params.length}`,
      );
