...
describe("checkpoint gradient correctness", () => {
  ...
  it("produces identical gradients for matmul", async () => {
    // Without checkpoint
    const a1 = api.tensorFromArray([1, 2, 3, 4], [2, 2], {
      requiresGrad: true,
    });
    const b1 = api.tensorFromArray([5, 6, 7, 8], [2, 2], {
      requiresGrad: true,
    });
    ...
    // With checkpoint
    const a2 = api.tensorFromArray([1, 2, 3, 4], [2, 2], {
      requiresGrad: true,
    });
    const b2 = api.tensorFromArray([5, 6, 7, 8], [2, 2], {
      requiresGrad: true,
    });
  ...
  it("produces identical gradients for softmax", async () => {
    // Without checkpoint
    const a1 = api.tensorFromArray([1, 2, 3, 4, 5, 6], [2, 3], {
      requiresGrad: true,
    });
    ...
    // With checkpoint
    const a2 = api.tensorFromArray([1, 2, 3, 4, 5, 6], [2, 3], {
      requiresGrad: true,
    });
  ...
  it("handles nested operations correctly", async () => {
    ...
    const c1 = api.tensorFromArray([0.1, 0.2, 0.3, 0.4], [2, 2], {
      requiresGrad: true,
    });
    ...
    const c2 = api.tensorFromArray([0.1, 0.2, 0.3, 0.4], [2, 2], {
      requiresGrad: true,
    });
    const y2 = checkpoint(
      api,
...
describe("checkpoint pack/unpack hooks", () => {
  ...
  it("pack hook is called for each saved tensor", () => {
    ...
    const _result = api.saved_tensors_hooks(
      (tensor) => {
