...
async function runTest(
  ...
  const totalAcquires =
    stats.acquireNew + stats.acquireFromPool + stats.acquireFromPending;
  const reuseRate =
    totalAcquires > 0
...
describe("Checkpoint Segmentation", { timeout: 300000 }, () => {
  ...
  it("segmented execution provides memory savings for checkpointed models", async () => {
    ...
    // Print results
    console.log(
      "| Configuration | Peak MB | New Allocs | From Pending | Reuse % |",
    );
    console.log(
      "|---------------|---------|------------|--------------|---------|",
    );
    ...
    console.log(
      `Early release only: ${((1 - earlyReleasePeak / baselinePeak) * 100).toFixed(1)}%`,
    );
    console.log(
      `Checkpoint + early release: ${((1 - ckptEarlyPeak / baselinePeak) * 100).toFixed(1)}%`,
    );
    console.log(
      `Additional benefit from checkpoint: ${((1 - ckptEarlyPeak / earlyReleasePeak) * 100).toFixed(1)}%`,
    );
  ...
  it("produces correct gradients with checkpoint and early release", async () => {
    ...
    console.log(
      `First 5 gradient values: ${gradValue
        .slice(0, 5)
        .map((v) => v.toFixed(4))
        .join(", ")}`,
    );
