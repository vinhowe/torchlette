...
describe("DistilGPT2 Full Finetuning Verification", () => {
  ...
  describe("Gradient Verification", () => {
    it(
      "all parameters receive gradients during full finetuning",
      { timeout: 60000 },
      async () => {
        if (!webgpuAvailable) {
  ...
  describe("Training Loop", () => {
    it(
      "5 training steps produce finite losses",
      { timeout: 600000 },
      async () => {
        if (!webgpuAvailable) {
          console.log("Skipping: WebGPU not available");
    ...
    it(
      "loss decreases or stays stable over training steps",
      { timeout: 600000 },
      async () => {
        if (!webgpuAvailable) {
          console.log("Skipping: WebGPU not available");
        ...
        const inputData = Array.from(
          { length: 32 },
          (_, i) => i % TEST_CONFIG.vocabSize,
        );
        const targetData = Array.from(
          { length: 32 },
          (_, i) => (i + 1) % TEST_CONFIG.vocabSize,
        );
  ...
  describe("Memory Stability", () => {
    it(
      "zero steady-state growth in all memory metrics",
      { timeout: 600000 },
      async () => {
        if (!webgpuAvailable) {
          console.log("Skipping: WebGPU not available");
        ...
        for (let i = 0; i < MEASURE_STEPS; i++) {
          ...
          console.log(
            `  step ${i}: allocs delta=${allocsAfter - allocsBefore}`,
          );
        }
        await drainGC();
        const s2 = snapshot();
        ...
        const rate = {
          ...
          trackerMB:
            (s2.trackerBytes - s1.trackerBytes) / (MEASURE_STEPS * 1e6),
        };

        console.log(
          `Per-step rate: storages=${rate.storages}, reachable=${rate.reachable}, allocs=${rate.allocs}, trackerMB=${rate.trackerMB.toFixed(2)}`,
        );
  ...
  describe("Combined Features", () => {
    it(
      "checkpoint + GradScaler work together (AMP disabled due to reshape issue)",
      { timeout: 300000 },
      async () => {
        if (!webgpuAvailable) {
          console.log("Skipping: WebGPU not available");
        ...
        await scaler.resolveDeferred();
        console.log(
          `Loss: ${lossVal.toFixed(4)}, Scale: ${scaler.getScale()}, Found inf: ${scaler.foundInf}`,
        );
      },
    );
  });
