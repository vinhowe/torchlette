...
import { Adam, Torchlette } from "../../src";
...
describe.skipIf(cpuOnly)("WebGPU Adam vec4 coalescing", () => {
  ...
  it(
    "vec4 path: 8-element param produces correct Adam update",
    { timeout: 30000 },
    async () => {
      ...
      const x = api.tensorFromArray([1, 2, 3, 4, 5, 6, 7, 8], [2, 4]);
  ...
  it(
    "vec4 path: 16-element param with weight decay (AdamW)",
    { timeout: 30000 },
    async () => {
  ...
  it(
    "scalar fallback: 6-element param still works correctly",
    { timeout: 30000 },
    async () => {
