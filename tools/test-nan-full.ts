...
async function main() {
  ...
  await api.markStep();
  // Full TinyShakespeare like the browser
  const text = fs.readFileSync("node_modules/.cache/tinyshakespeare.txt", "utf-8");
  console.log("Training on " + text.length + " chars");
  const trainer = new LoRATrainer(api, model, tok);
  await trainer.train(text, {
    maxSteps: 50, batchSize: 1, seqLength: 128, learningRate: 1e-3,
    useAMP: false, useCheckpointing: false,
  }, { onStepEnd: (s, l) => {
    if (isNaN(l)) { console.log("NaN at step " + s); process.exit(1); }
    if (s % 10 === 0 || s === 49) console.log("step " + s + ": " + l.toFixed(4));
  }});
