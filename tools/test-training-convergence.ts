/**
 * Test: does GPT-2 small LoRA training converge with the default init?
 * No init overrides — uses whatever lora.ts produces.
 */
...
async function main() {
  ...
  console.log(`Tokens: ${tokens.length}, rank=64, default randn*0.01 init\n`);
  ...
  console.log("\n=== Generation ===");
  model.train(false);
  for (const prompt of ["First Citizen:", "We are", "MENENIUS:"]) {
    let gen = "";
