...
const FIXED_TOKENS = [
  15496, 11, 314, 1101, 257, 3303, 2746, 2746, 290, 314, 1101, 994, 284, 1037,
  13, 314, 765, 284, 1037, 546, 262, 995, 290, 787, 340, 257, 1365, 1295, 13,
  314, 1101, 257,
];
...
async function main() {
  ...
  for (let step = 0; step < NUM_STEPS; step++) {
    ...
    console.log(
      `Step ${step}: loss=${lossValue.toFixed(4)} | fwd=${(t1 - t0).toFixed(0)}ms bwd=${(t2 - t1).toFixed(0)}ms opt=${(t3 - t2).toFixed(0)}ms cleanup=${(t4 - t3).toFixed(0)}ms | total=${(t4 - t0).toFixed(0)}ms`,
    );
  }
