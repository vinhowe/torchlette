...
async function main() {
  ...
  console.log(
    `EMB [0:10]:`,
    embArr.slice(0, 10).map((v) => v.toFixed(8)),
  );
  ...
  for (let i = 0; i < numLayers; i++) {
    ...
    console.log(
      `L${i} [0:10]:`,
      layerArr.slice(0, 10).map((v) => v.toFixed(8)),
    );
    // Also print values at offset embedDim (start of position 1)
    console.log(
      `L${i} [${model.config.embedDim}:${model.config.embedDim + 5}]:`,
      layerArr
        .slice(model.config.embedDim, model.config.embedDim + 5)
        .map((v) => v.toFixed(8)),
    );
