...
async function runStep(
  api: Torchlette,
  compiledForward: (...args: Tensor[]) => Tensor,
  optimizer: Adam,
  scaler: GradScaler | null,
) {
  if (scaler) await scaler.resolveDeferred();
  await api.beginStep();
  const input = api.tensorFromArray(FIXED_TOKENS.slice(0, -1), [1, SEQ_LEN], {
    device: "webgpu",
  });
  const target = api.tensorFromArray(FIXED_TOKENS.slice(1), [1, SEQ_LEN], {
    device: "webgpu",
  });
...
async function main() {
  ...
  for (let i = 1; i <= 4; i++) {
    ...
    console.log(
      `  Step ${i}: ${(performance.now() - ts).toFixed(0)}ms (loss=${l.toFixed(4)})`,
    );
  }
  ...
  console.log(
    `  Parallel (createComputePipelineAsync): ${warmupTime.toFixed(0)}ms (${warmupResult.compiled} compiled, ${warmupResult.skipped} skipped)`,
  );
  ...
  console.log(
    `  Sequential (createComputePipeline):    ${syncTime.toFixed(0)}ms`,
  );
  ...
  console.log(
    `  Step 0 wall clock (includes all overhead): ${step0Time.toFixed(0)}ms`,
  );
  console.log(`  Pipeline compilation only:`);
  console.log(
    `    Sequential:  ${syncTime.toFixed(0)}ms (${registry.length} shaders)`,
  );
  console.log(
    `    Parallel:    ${warmupTime.toFixed(0)}ms (${registry.length} shaders)`,
  );
  ...
  console.log(
    `    With parallel warmup:      ${(nonCompileTime + warmupTime).toFixed(0)}ms`,
  );
  console.log(`    vs baseline:               ${step0Time.toFixed(0)}ms`);
  console.log(
    `    Step 0 speedup:            ${(step0Time / (nonCompileTime + warmupTime)).toFixed(2)}×`,
  );
