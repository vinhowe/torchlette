...
const { TraceMap, originalPositionFor } = require(
  path.join(
    rootDir,
    "node_modules/.pnpm/@jridgewell+trace-mapping@0.3.30/node_modules/@jridgewell/trace-mapping",
  ),
);
...
const rawMap = JSON.parse(
  fs.readFileSync("/tmp/profile-bundle.mjs.map", "utf8"),
);
...
const startIdx = Math.floor(samples.length * 0.7);
...
for (let i = startIdx; i < endIdx; i++) {
  ...
  let key;
  if (cf.url?.includes("profile-bundle.mjs")) {
    ...
    key =
      (cf.functionName || "(anon)") + " @ " + file + ":" + (cf.lineNumber + 1);
  }
...
for (let i = 0; i < 50 && i < sorted.length; i++) {
  ...
  cum += time;
  console.log(
    (time / 1000).toFixed(1).padStart(8) +
      "ms (" +
      ((time / totalTime) * 100).toFixed(1).padStart(5) +
      "%, cum " +
      ((cum / totalTime) * 100).toFixed(1).padStart(5) +
      "%)  " +
      key,
  );
}
