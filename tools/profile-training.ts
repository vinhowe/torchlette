...
} from "../src/backend/webgpu/bind-group-cache";
...
function instrumentedForwardWithLoss(
  ...
  for (let i = 0; i < model.h.length; i++) {
    ...
    const q = qSlice
      .reshape([batch, sl, numHeads, headDim])
      .permute([0, 2, 1, 3])
      .contiguous();
    const k = kSlice
      .reshape([batch, sl, numHeads, headDim])
  ...
  const realLogits =
    model.paddedVocabSize > model.config.vocabSize
...
async function main() {
  if (!isProfilingEnabled()) {
    ...
    console.error(
      "Usage: TORCHLETTE_PROFILE=1 npx tsx tools/profile-training.ts",
    );
    process.exit(1);
  }
  ...
  if (!gpuOk) {
    console.error(
      "ERROR: WebGPU initialization failed (is the GPU available?)",
    );
    process.exit(1);
  }
  ...
  console.log(
    `Batch size: ${BATCH_SIZE}, Sequence length: ${FIXED_TOKENS.length - 1}\n`,
  );
  ...
  if (fs.existsSync(warmupFile)) {
    ...
    console.log(
      "No pipeline cache found — step 0 will compile all pipelines.\n",
    );
  }
  ...
  for (let step = 0; step < NUM_STEPS; step++) {
    ...
    const target = api.tensorFromArray(targetData, [BATCH_SIZE, seqLen], {
      device: "webgpu",
    });
    ...
    const _lastStats = api._runtime().getLastFusionStats();
    ...
    if (step === 0) {
      ...
      console.log(
        `  Pipeline registry: ${pipelineRegistry.length} pipelines saved to ${warmupFile}`,
      );
    }
    ...
    if (step === PROFILE_STEP) {
      ...
      if (memStats) {
        console.log(
          `GPU Memory: ${(memStats.currentBytes / 1024 / 1024).toFixed(1)}MB current / ${(memStats.limitBytes / 1024 / 1024).toFixed(0)}MB limit (${memStats.allocationCount} live buffers)`,
        );
        console.log(
          `Peak Memory: ${(memStats.peakBytes / 1024 / 1024).toFixed(1)}MB (${memStats.usagePercent.toFixed(1)}% current utilization)`,
        );
      }
      if (poolStats) {
        console.log(`\nBuffer Pool:`);
      ...
      if (detailedStats) {
        ...
        console.log(
          `  Acquire from pool: ${detailedStats.acquireFromPool} | from pending: ${detailedStats.acquireFromPending} | new alloc: ${detailedStats.acquireNew}`,
        );
        console.log(
          `  Release to pool: ${detailedStats.releaseToPool} | to destroy: ${detailedStats.releaseToDestroy}`,
        );
      }
            ...
            stats.totalBytes >= 1024 * 1024 * 1024
              ...
              : stats.totalBytes >= 1024 * 1024
                ...
                : stats.totalBytes >= 1024
                  ...
                  : `${stats.totalBytes}B`;
          console.log(
            `  ${bucket.padEnd(10)} ${String(stats.count).padStart(4)} buffers (${bytesStr.padStart(8)})`,
          );
        }
      }
      console.log();
      ...
      console.log(
        `Bind Group Cache: ${bgStats.hits} hits / ${bgStats.misses} misses (${(bgStats.hitRate * 100).toFixed(1)}%), ${bgStats.size} cached entries`,
      );
      const arenaStats = getArenaResolveStats();
      console.log(
        `Arena resolve: ${arenaStats.hits} hits, ${arenaStats.aliased} aliased (fallthrough), ${arenaStats.noArena} no-arena`,
      );
      const missLog = getBindGroupCacheMissLog();
      if (missLog.length > 0) {
        console.log(`\nBind Group Cache Misses (step 4):`);
        console.log(
          `${"Idx".padStart(5)}  ${"Reason".padEnd(20)}  ${"Label".padEnd(28)}  Details`,
        );
        console.log(
          `${"─".repeat(5)}  ${"─".repeat(20)}  ${"─".repeat(28)}  ${"─".repeat(40)}`,
        );
        for (const m of missLog) {
          console.log(
            `${String(m.idx).padStart(5)}  ${m.reason.padEnd(20)}  ${(m.label ?? "(null)").padEnd(28)}  ${m.details}`,
          );
        }
      }
      console.log();
      ...
      if (cumulativeStats) {
        ...
        console.log(
          `Fused nodes: ${cumulativeStats.fusedNodes} (${((cumulativeStats.fusedNodes / cumulativeStats.totalNodes) * 100).toFixed(1)}%)`,
        );
      ...
      console.log(
        `Step ${step}: loss=${lossValue.toFixed(4)}, fwd=${(t1 - t0).toFixed(0)}ms, bwd=${(t2 - t1).toFixed(0)}ms, opt=${(t3 - t2).toFixed(0)}ms, cleanup=${(t4 - t3).toFixed(0)}ms | mem=${stepMem.currentMB.toFixed(1)}MB, pool=${stepMem.poolReuseRate.toFixed(1)}%, new_alloc=${stepPoolStats.acquireNew}`,
      );
    }
  }
  ...
  if (steadyState.length > 0) {
    ...
    console.log(
      `  Forward:  ${avg(steadyState.map((t) => t.fwd)).toFixed(0)}ms`,
    );
    console.log(
      `  Backward: ${avg(steadyState.map((t) => t.bwd)).toFixed(0)}ms`,
    );
    console.log(
      `  Optimizer: ${avg(steadyState.map((t) => t.opt)).toFixed(0)}ms`,
    );
    console.log(
      `  Cleanup:  ${avg(steadyState.map((t) => t.cleanup)).toFixed(0)}ms`,
    );
    const totalAvg = avg(
      steadyState.map((t) => t.fwd + t.bwd + t.opt + t.cleanup),
    );
  ...
  if (stepMemory.length >= 3) {
    ...
    if (n > 0) {
      const storagesDelta =
        (stepMemory[steadyEnd].storages - stepMemory[steadyStart].storages) / n;
      const reachableDelta =
        (stepMemory[steadyEnd].reachable - stepMemory[steadyStart].reachable) /
        n;
      const trackerDelta =
        (stepMemory[steadyEnd].currentMB - stepMemory[steadyStart].currentMB) /
        n;
      const allocDelta =
        (stepMemory[steadyEnd].allocCount -
          stepMemory[steadyStart].allocCount) /
        n;
      ...
      console.log(
        `  CurrentMB/step: ${trackerDelta >= 0 ? "+" : ""}${trackerDelta.toFixed(1)}`,
      );
      console.log(
