...
5. **Backward fusion: the win lives at CLAIM altitude, not the elementwise detector** (REPRICED 2026-07-10, islands I2b — measured on real A100 plans). The old "consecutive-only detector is the limiter" premise is FALSIFIED: the Kahn reorder + claim cascade already fuse ~90% of unclaimed fusibles; the residual is matmul-adjacent length-1/2 cast runs (gelu→cast | transpose→matmul), unmergeable at elementwise altitude. The real targets: (a) prologue-claim casts into the adjacent matmul dispatch (islands I3 territory), (b) reduction batching for bias-grad sums (target #3). NOTE: the cumulative fusion stat is REPLAY-BLIND (increments only on the lowered path) — never quote it as a fusion rate; use per-template analysis. A gap-spanning detector extension was built, proven sound, measured NULL, and reverted (islands-design docs) — do not re-attempt without a new workload.
