...
1. **Matmul kernel optimization** — 45.0ms GPU (68% of total). Bare backward matmuls alone are 25.5ms (39%). Specific sub-targets:
   - **Per-shape autotuning** (~5-10% potential): Current tile configs are per-shape-class (square_large, large_k, etc.). Runtime autotuning benchmarks multiple configs for each actual shape encountered, picking the best. Already have infrastructure (`getConfigForShape`/tuning cache) but defaults may not be optimal.
