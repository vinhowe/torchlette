...
Steady-state ~51ms/step wall clock (profiler step 4, lowered plan cached, no replay). GPU: ~78ms total. Top GPU kernels: `matmul` 24.8ms (31.8%), `fusedAttentionBackward` 15.5ms (19.9%), `matmul++cast+bias+binary` 9.1ms (11.7%), `fusedAttentionForward` 5.9ms (7.6%), `adamStep` 4.8ms (6.2%), `matmul++cast+bias+unary+cast` 4.0ms (5.1%), `matmul++cast+bias` 3.6ms (4.6%), `matmul++bias` 2.2ms (2.8%). Cross-entropy fwd+bwd 1.1ms (1.4%). Bind group cache 100.0% (0 misses). 10 submits/step. Fusion: 1156 nodes, 183 fused (15.8%), 20 groups. Memory: 5489MB current / 5489MB peak / 32GB limit. Pool reuse 52.1%. 1 new alloc/step. Leak status: OK (storages +0.0/step).
...
1. **fusedAttentionBackward further tuning** — 15.5ms GPU (19.9%). Max 1645µs/dispatch, 24 dispatches. BQ=32 doesn't help (shared memory L/D reads faster than global; register pressure with larger tiles). Needs different algorithmic approach.
2. **GC pressure** — Object pooling for Tensor metadata.
3. **Pipeline cache warmup** — Step 0 is 3.0s (60× steady-state) due to pipeline compilation. Pre-compile pipeline variants during model load.
