...
- **GPT-2 Medium 512**: **OOMs (>32GB) in default mode** on V100 — the per-position arena exceeds memory. With `TORCHLETTE_ARENA_LIVENESS=1`: fits at 12.3GB peak but ran ~518ms/step while compiled plans were disabled in that mode. **FIXED 2026-06-10 — planned compiled buffers** (ON by default under liveness; `TORCHLETTE_COMPILED_PLANNED=0` opts out): compiled replays pin and rebind the recorded pool-buffer assignment. V100 (sivri) measurements: Medium@512 **13.8GB peak at ~198ms/step** (default arena: 28.6GB/~210ms — fits the 32GB V100 with nothing to spare; liveness lowered: 12.3GB/~525ms); DistilGPT-2 5.0GB peak (was 9.1GB) at the same speed; 124M regression baselines hold at 2.85GB peak (was 5.3GB). Liveness+planned is strictly better than default mode on these workloads — candidate to become the overall default after broader soak.
