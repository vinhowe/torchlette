...
1. **Matmul kernel optimization** — 45.0ms GPU (68% of total). Bare backward matmuls alone are 25.5ms (39%).
...
5. **CSE/DCE in the LazyIRNode pipeline** — Add general-purpose Common Subexpression Elimination and Dead Code Elimination as rewrite passes in `graph-compiler.ts`'s `analyzeGraph()`, operating on `LazyIRNode[]` + consumer maps (same infrastructure as `runRewritePasses()`). **CSE**: nodes with identical `(op, input node IDs, payload, shape, dtype)` are merged by redirecting consumers to the first occurrence; RNG ops excluded per §15. **DCE**: nodes with zero consumers that aren't outputs or effectful (in-place ops) are removed. Both passes are ~30-50 lines each using the existing `bypassed` set mechanism from `graph-rewrites.ts`. Note: the existing `ir-optimize.ts` implements CSE/DCE on `IRGraph` (the §8 compile-time tracing IR), but `IRGraph` doesn't drive execution — `compile()` just toggles fusion on the `LazyIRNode` path. The new passes should operate directly on `LazyIRNode`. Impact unknown — needs instrumentation of a DistilGPT-2 training step to measure how many duplicate subexpressions and dead nodes survive to execution after the existing domain-specific rewrites.
