...
// ============================================================================
// Compilation: build CompiledPlan
// ============================================================================
 ...
 * corruption), and final plan assembly + registry ownership. Called by the
 * generated build (buildCompiledPlanFromGenerated) — the planner sees only the
 * command stream + result slots.
 */
...
/**
 * Encode an INTRA-PLAN buffer-to-buffer copy on the shared encoder — one whose
 * destination is later read by a dispatch in the same plan (scatter-add
 * accumulator, cat assembly, matmul k-split temp, packed-optimizer pack/unpack).
 * The generated-stream builder derives the corresponding TAG_COPY from the IR,
 * so callers route these through this helper (rather than raw
 * enc.copyBufferToBuffer) to keep the single "intra-plan copy" call site.
 *
